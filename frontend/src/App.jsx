...
function App() {
	...
	let onMessageArrived = (message) => {
		...
		altitudeChartRef.current.data.datasets[0].data.push({x: time, y:newData[2]});
		altitudeChartRef.current.data.datasets[1].data.push({x: time, y:newData[1]});
		altitudeChartRef.current.update('quiet');
		//
		velocityChartRef.current.data.datasets[0].data.push({x: time, y:newData[3]});
		velocityChartRef.current.update('quiet');
		//
		accelerationChartRef.current.data.datasets[0].data.push({x: time, y:newData[4]});
